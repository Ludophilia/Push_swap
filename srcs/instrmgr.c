...
/*   Updated: 2025/03/13 20:26:24 by jegerman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "pushswap.h"

static int	imgr_chk_instrs(char *curr_it, char *next_it, char *inst_ref1,
	char *inst_ref2)
{
	size_t	ref_len;
	int		inst_test[2];
...
int	imgr_opti_instrs(t_list **instrs)
{
	...
	while (inst_node && inst_node->next)
	{
		...
		if ((imgr_chk_instrs(curr_it, next_it, "ra", "rb")
				&& imgr_smp_instr("rr", inst_node) == -1)
			|| (imgr_chk_instrs(curr_it, next_it, "sa", "sb")
				&& imgr_smp_instr("ss", inst_node) == -1)
			|| (imgr_chk_instrs(curr_it, next_it, "rra", "rrb")
