...
/*   Updated: 2025/02/28 15:01:53 by jegerman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "pushswap.h"

static int	sort1000_update_candidates(t_list *first_a, t_list *next_a,
	t_list *node_b, t_cnd *cands)
{
	if (next_a == NULL && get_nb(node_b) < get_nb(first_a))
	{
		cands->nba = get_nb(first_a);
		cands->nbb = get_nb(node_b);
		cands->rots_tt = cands->rots_a + cands->rots_b;
	}
	else if (next_a && get_nb(node_b) > get_nb(first_a)
...
static int	sort1000_search_candidates_in_b(t_list *first_a, t_list *next_a,
	t_stk *stack_b, t_cnd *cands)
{
	...
	while (node_b)
	{
		...
		if (cands->rots_a + cands->rots_b < cands->rots_tt)
			sort1000_update_candidates(first_a, next_a, node_b, cands);
		node_b = node_b->next;
	}
	return (0);
}
...
static t_cnd	sort1000_search_candidates(t_stk *stack_a, t_stk *stack_b)
{
	...
	while (nodes_a.first && cands.rots_tt != 0)
	{
		...
		if (sort_node_is_min(nodes_a.first, stack_a))
			sort1000_search_candidates_in_b(nodes_a.first, NULL, stack_b, &cands);
		if (get_nb(nodes_a.first) + 1 != get_nb(nodes_a.next))
			sort1000_search_candidates_in_b(nodes_a.first, nodes_a.next, stack_b, &cands);
		nodes_a.first = nodes_a.first->next;
